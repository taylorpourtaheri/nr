...
plot1 <- ggplot(ggnetwork(network), aes(x = x, y = y, xend = xend, yend = yend)) +
...
ggsave(plot = plot1, filename = 'figure1.png', width = 20, height = 20)
...
plot2 <- ggplot(ggnetwork(network), aes(x = x, y = y, xend = xend, yend = yend)) +
    geom_edges(alpha = 0.7) + 
    geom_nodes(aes(color = logFC, size = betweenness), alpha = 0.65) +
    geom_nodetext_repel(aes(label = Symbol)) + 
    scale_color_gradient(low = 'blue', high = 'red') +
...
ggsave(plot = plot2, filename = 'figure2.png', width = 20, height = 20)
